# Feishu DetailPage Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development to implement this plan task-by-task.

**Goal:** Create a two-column Tab-based Feishu channel configuration UI supporting multiple AppId/AppSecret accounts with global settings.

**Architecture:**
- FeishuDetailPage.tsx component with Accounts and Settings tabs
- Left sidebar for account list, right panel for account details (similar to Telegram)
- Global settings in Settings tab covering DM policy, group policy, quotas, streaming
- Type-safe React component with real-time form updates

**Tech Stack:** React 18, TypeScript, shadcn/ui (Tabs, Input, Select, Switch), Tailwind CSS, Lucide icons

---

## Task 1: Add Feishu Type Definitions

**Files:**
- Modify: `src/types.ts:43-50` (after TelegramChannelConfig)

**Step 1: Add Feishu type interfaces to types.ts**

After the TelegramChannelConfig interface, add:
...
git commit -m "types: add FeishuAccountConfig and FeishuChannelConfig interfaces"
```

---

## Task 2: Create FeishuDetailPage Component - Basic Structure

**Files:**
- Create: `src/components/editor/FeishuDetailPage.tsx`
...
Run: `pnpm tsc --noEmit src/components/editor/FeishuDetailPage.tsx`
Expected: No errors

**Step 3: Commit**

```bash
git add src/components/editor/FeishuDetailPage.tsx
git commit -m "feat: create FeishuDetailPage base component structure"
```
...
**Files:**
- Modify: `src/components/editor/FeishuDetailPage.tsx:47-63` (add functions)

**Step 1: Add account management functions to FeishuDetailPage**

Inside FeishuDetailPage function, after state declarations, add:
...
```typescript
  ...
  function removeAllowFrom(userId: string) {
    const current = fs.allowFrom ?? []
    patchFeishu(config, { allowFrom: current.filter(u => u !== userId) }, onChange)
  }
  ...
  function removeGroupAllowFrom(groupId: string) {
    const current = fs.groupAllowFrom ?? []
    patchFeishu(config, { groupAllowFrom: current.filter(g => g !== groupId) }, onChange)
  }
...
## Task 4: Implement Accounts Tab - Left Column (Account List)

**Files:**
- Modify: `src/components/editor/FeishuDetailPage.tsx` (replace placeholder tabs section)

**Step 1: Add Tabs component and left column**

Replace the placeholder tabs section (from "Placeholder for tabs" onwards) with:
        ...
        <Tabs defaultValue="accounts" className="w-full">
          ...
          <TabsContent value="accounts" className="flex-1 overflow-hidden">
            <div className="grid grid-cols-3 gap-3 p-4 h-[calc(100%-56px)] overflow-hidden">
              ...
              {/* Right side - Account details will be added in next task */}
              <div className="col-span-2 border rounded-lg bg-white">
                <p className="p-4 text-gray-500">Account details will be shown here</p>
              </div>
            </div>
          </TabsContent>
...
```bash
git add src/components/editor/FeishuDetailPage.tsx
git commit -m "feat: implement Accounts tab left column with account list"
```

---

## Task 5: Implement Accounts Tab - Right Column (Account Details)

**Files:**
- Modify: `src/components/editor/FeishuDetailPage.tsx` (replace "Account details will be shown here")

**Step 1: Replace the right column placeholder with account details form**
...
```bash
git add src/components/editor/FeishuDetailPage.tsx
git commit -m "feat: implement Accounts tab right column with account details form"
```
...
**Files:**
- Modify: `src/components/editor/FeishuDetailPage.tsx` (replace Settings tab placeholder)
...
**Step 2: Verify compilation and update "Update account" button**

Update the add account button onClick to call the `addAccount` function that was created in Task 3:
...
## Task 7: Integrate FeishuDetailPage into ChannelsTab

**Files:**
- Modify: `src/components/editor/ChannelsTab.tsx`

**Step 1: Read ChannelsTab to understand structure**

Current structure should show navigation between ChannelsListPage and TelegramDetailPage.

**Step 2: Add Feishu import and navigation**
...
In the conditional rendering where it shows TelegramDetailPage, add a similar condition for Feishu:

```typescript
{view === "detail" && selectedChannel === "telegram" && (
  <TelegramDetailPage
    config={config}
    onChange={onChange}
...
{view === "detail" && selectedChannel === "feishu" && (
  <FeishuDetailPage
    config={config}
    onChange={onChange}
    onBack={() => { setView("list"); setSelectedChannel(null) }}
...
git commit -m "feat: integrate FeishuDetailPage into ChannelsTab navigation"
```

---

## Task 8: Update ChannelsListPage to Enable Feishu

**Files:**
- Modify: `src/components/editor/ChannelsListPage.tsx:26-31` (update getChannelStatus function)

**Step 1: Enable Feishu channel in list**

Update the `getChannelStatus` function to recognize Feishu as enabled:
...
git commit -m "feat: enable Feishu channel status in ChannelsListPage"
```
...
Expected: Should see all 8 commits from tasks above
...
- ✅ FeishuDetailPage.tsx created with full functionality
...
- ✅ DM policy with allowlist validation
- ✅ Group policy with conditional allowlist
- ✅ Quota optimization toggles
- ✅ Streaming options
- ✅ FeishuDetailPage integrated into ChannelsTab
- ✅ Feishu channel clickable from ChannelsListPage
...
2. Click "飞书 (Feishu)" in channel list
3. Should see Feishu detail page with Accounts and Settings tabs
4. **Accounts tab:**
   - Click "+ 添加账户" button → new account appears in list
   - Click account in list → details show in right panel
   - Edit App ID, App Secret, Bot Name → changes persist
   - Toggle Enabled → changes persist
   - Click trash icon → account deleted, next account auto-selected
5. **Settings tab:**
   - Toggle Enable Feishu
   - Change DM Policy → allowlist shows when selected
   - Add open_id to allowlist → tag appears
   - Remove tag → tag disappears
   - Change Group Policy → allowlist shows when selected
   - Toggle Quota Optimization switches
   - Toggle Streaming switches
